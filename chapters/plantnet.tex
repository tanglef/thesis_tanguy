...
Currently, the weight of user $j$, denoted by $w_j$ is computed from the estimated number of identified species $n_j$.
This choice does not penalize users who
\begin{itemize}
...
To mitigate this issue, classically in crowdsourcing, the votes of other users can be used to correct the mistakes and a time-dependency can be introduced to the weights.
However, as presented before, the sparsity of the votes is very high in Pl@ntNet's database.
