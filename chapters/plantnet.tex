...
In Pl@ntNet, contributions are made by volunteers and the tasks are not paid.
...
Computer vision models are a great aid in plant species recognition in the field \citep{vidal2021perspectives,mader2021flora,borowiec2022}.
...
\begin{itemize}
        ...
        \item Division: separates spore (angiosperms) or seed (gymnosperms) reproduction with specific characteristics. There are 14 plant divisions in total.
        ...
        \item Order: Group of families with common characteristics -- \emph{e.g} Cucurbitales (generally ends with \emph{-ales}).
        \item Family: Plants with similar flower, fruit and seed structures -- \emph{e.g} Begonaias and Allies.
...
In the case a plant is a hybrid a cross between two species is added -- it is written with an × as in \emph{Begonia} × \emph{semperflorens}. The position of the × indicates if it is the hybridization between two genera or species.
...
The vote is guided by the computer vision model predictions (see \Cref{sub:pipeline_plantnet}) and other pictures are selected via a similarity search algorithm.
...
Note that this identification can later be invalidated by other users with enough weight thanks to the accuracy threshold $\theta_{\text{accs}}$.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        Each user must have at least $n_2=n_3=40$ identified species to validate their correction if $n_1=30$, or they can compensate each other knowledge (see \Cref{fig:validate_again}). This requirement is hardly met in practice, leading to a similar accuracy for users.
        \begin{figure}[H]
            \centering
