...
For example, the label recovery accuracy $\mathrm{Acc}$ on $\mathcal{D}_\text{disagreement}$ is given in \Cref{tab:res_disagreement_weights}.
...
\begin{figure}[htb]
    ...
    \caption{Zoomed empirical CDF of the weights obtained after running Pl@ntNet's label aggregation strategy with the weight functions tested in \Cref{tab:weight_functions}. The $\log$ transformation results is represented for the sake of completeness.}
