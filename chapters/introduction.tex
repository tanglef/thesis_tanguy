...
The literature has mainly considered modeling workers abilities and faults \citep{dawid_maximum_1979, raykar_ranking_2011, sinha2018fast,rodrigues2018dee}, but very few models incorporate the tasks' difficulty \citep{whitehill_whose_2009, chu2021learning}.
...
In this work, we first propose in \Cref{chap:waum} to identify ambiguous tasks in training datasets from the crowdsourced labels and the actual image with a metric called the $\mathrm{WAUM}$ (Waighted Areas Under the Margin) introduced in \citet{lefort2022improve}.
Defining what is a difficult task proves to be a challenge in itself.
An informal definition of a difficult task is given in \citet{angelova2004data}:
\begin{center}
\begin{minipage}{.75\textwidth}
\emph{
...
To transform these scores into probabilities, we use the softmax function $\sigma(\bigcdot)$ defined as:
$$
...
The indicator function is denoted $\mathds{1}(\bigcdot)$. The simplex of dimension $K-1$ is written as $\Delta_K=\{p \in\mathbb{R}^K_+ | \sum_{k=1}^K p_k = 1\}$.
...
\WHILE{Not converged}
    ...
    \STATE Normalize $P(T_{i,\bigcdot} = 1 | Y, \pi, \rho)$
...
However, contrary to models seen in \Cref{sub:aggregating_votes}, these strategies are only used for prediction and not for aggregating labels for each task.
