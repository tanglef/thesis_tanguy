...
\subsubsection{Datasets, solver and library information}
...
\end{equation}
with $n=\sum_{c\in [K]} n_c$, $n_c=\sum_{k}o_{c,k}$ for $k\in [K]$ and
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        ...
        \hline
        \hline
        AudioBirds \citep{lehikoinen2023successful} & 205 & 79592 & 2 & 0.81 \\
        bluebirds \citep{WelinderEtal10b} & 50 & 300 & 2 & 0.126 \\
        relevance-2 \citep{CrowdKit2023} & 120 & 600 & 2 & 0.262 \\
        CIFAR-10H \citep{peterson_human_2019} & 200 & 1000 & 10 & 0.91 \\
        LabelMe \citep{rodrigues2014gaussian} & 150 & 800 & 8 & 0.677 \\
        Music \citep{rodrigues2017learning} & 80 & 400 & 10 & 0.301 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{align*}
    ...
    \text{with } \beta_j &= \frac{1}{|\{y_{i'}^{(j)}\}_{i'}|} \sum_{i'=1}^{n_{\mathrm{task}}} \mathds{1}\left(y_{i'}^{(j)} = \mathrm{MV}(\{y_{i'}^{(j)}\}_j)\right)\enspace.
    \end{align*}
